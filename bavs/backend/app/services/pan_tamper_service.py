"""
PAN Tampering Detection Service — Module 3 orchestration layer.

Pipeline (identical to Module 2's Aadhaar pipeline, per locked spec):
    PAN Image -> Preprocessing -> ELA Generation -> Feature Extraction
    (OCR + ELA + ResNet18) -> Feature Fusion -> Random Forest ->
    REAL or TAMPERED

This service reuses every component from app/ml/forgery/*.py and
app/ml/ocr/*.py without modification — those modules are document-type-
agnostic by design (see Module 2 delivery notes). The ONLY difference
from aadhaar_tamper_service.py is the model artifact path: PAN documents
have different visual layout/statistics than Aadhaar, so a separate
Random Forest is trained and loaded (PAN_MODEL_PATH vs AADHAAR_MODEL_PATH)
rather than sharing one classifier across both document types.

Train the PAN model via the same CLI script used for Aadhaar, pointed at
PAN sample data:
...
CRITICAL SAFETY BEHAVIOR: identical contract to Module 2 — if no trained
Random Forest model exists for PAN at the configured path, this service
does NOT fabricate a verdict. It persists a TamperingResult row with
verdict=INCONCLUSIVE, mirroring the ModelNotTrainedError handling in
aadhaar_tamper_service.py.
"""
...
PAN_MODEL_PATH = Path("ml_models/pan_rf_model.pkl")

_cached_model: ForgeryRandomForestModel | None = None


class PanTamperingServiceError(Exception):
    """Raised on unrecoverable errors during PAN tampering detection."""


def _get_pan_model() -> ForgeryRandomForestModel:
    """
    Lazily load and cache the PAN Random Forest model from disk.
    ...
    if _cached_model is None:
        _cached_model = ForgeryRandomForestModel.load(PAN_MODEL_PATH)
    return _cached_model


def analyze_pan_tampering(file_bytes: bytes) -> dict:
    """
    Run the full PAN tampering detection pipeline on raw image bytes.
    ...
    Raises:
        PanTamperingServiceError: on image loading or OCR/ELA/ResNet
            extraction failure.
    ...
    try:
        ...
        raise PanTamperingServiceError(f"Could not load image: {exc}") from exc
    ...
    try:
        ...
        raise PanTamperingServiceError(
...
def persist_tampering_result(
    ...
    """
    Persist a tampering analysis result for a PAN document. verdict must
    be one of TamperVerdict's values ("REAL", "TAMPERED", "INCONCLUSIVE").

    resnet_feature_vector_ref (added in Module 8): R2 object key pointing
    to the full 523-dim fused feature vector. See
    aadhaar_tamper_service.persist_tampering_result's docstring for full
    rationale (identical design, mirrored here for PAN).

    Note: this writes to the same `tampering_results` table used by
    Module 2 (Aadhaar) — the table is document-type-agnostic since
    document_id already disambiguates via the documents.document_type
    column. No schema duplication needed.
    """
    document = db.get(Document, document_id)
    if document is None:
        raise PanTamperingServiceError(f"Document {document_id} does not exist.")
...
def run_and_persist_pan_tampering_check(
    ...
    """
    try:
        analysis = analyze_pan_tampering(file_bytes)
        ...
        try:
            ...
            feature_vector_ref = upload_bytes(
                vector_payload,
                key_prefix=f"explainability/feature_vectors/pan/{document_id}",
        ...
        logger.warning(
            "No trained PAN tampering model available for document %s: %s. "
    ...
    # Module 9 wiring: single audit-write code path covers both branches.
    document = db.get(Document, document_id)
    write_audit_log(
        db,
        AuditEventType.PAN_TAMPERING_CHECK_COMPLETED,
